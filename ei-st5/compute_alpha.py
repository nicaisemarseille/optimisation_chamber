...
def compute_alpha(omega, material):
    ...
    resolution = 30  # := number of elements along L

    # Matériaux absorbant EI
    eps_r = 1.4
    sigmaelec = 0.718   # à modifier
    permeability_vacuum = 4* numpy.pi * 1e-3
    c_0 = 3e8
    rho_0 = 1.2e-3

    # Paramètres dans l'air
    mu_0 = 1.0
    ksi_0 = 1.0 / (c_0**2)
    gamma_p = 7.0/5.0
    phi = eps_r/gamma_p
    alpha_h = phi
    # Paramètres du matériaux absorbant
    mu_1 = 1.0
    ksi_1 = eps_r / (c_0 ** 2) 
    sigma = sigmaelec * (permeability_vacuum**2)*rho_0*(gamma_p*(phi**2))
    print("sigma:-----------------", sigma)
    print("sigmaelec:-----------------", sigmaelec)
    print("permeability_vacuum:--------------", permeability_vacuum**2)
    print("rho_0:-------------------", rho_0)
    print("gamma_p:------------", gamma_p)
    #exit(-2)
    a = sigma * (phi ** 2) * gamma_p #/ ((c_0 ** 2) * rho_0 * alpha_h)
    print("a----------:", a)
    print("((c_0 ** 2) * rho_0 * alpha_h):------------",  c_0 ** 2)
    #exit(-2)
    ...
    def g(y):
        ...
        mu_55 = 5 
        A = 1 #amplitude
        return A*numpy.exp(-(y-mu_55)**2/(2*sigma_55**2))
    ...
    def g_k(k):
        l = 5
        def integrand(y):
            return g(y) * numpy.exp(-1j * k * y)
        # Intégration sur [-l0, l0] pour les parties réelle et imaginaire
        rp, _ = quad(lambda y: integrand(y).real, -l, l)
        ip, _ = quad(lambda y: integrand(y).imag, -l, l)
        # Coefficient de Fourier c_n
        c_n = (rp + 1j * ip)/(2*l)
        # Mode de Fourier associé
        return c_n
    ...
    def alpha(omega):
        ...
        temp = real_to_complex(minimize(lambda z: numpy.real(sum_e_k(omega)(real_to_complex(z))), complex_to_real(alpha_0), tol=1e-4, method='Nelder-Mead').x)
